...
- **Decoration** — Procedural appearance generation. Tier system (S/A/B/C/D) controls gear quality. Three layers: body (face/hair/skin), equips (job-specific), NX overlay (20% gate, tier-scaled intensity).
...
- **Dialogue** — YAML-driven per-bot dialogue with randomized selection, emote IDs, timing, and variable substitution. 14 dialogue files.
...
- **Environment** — 7-wave phased world startup with inter-wave delays for stability.

## World Startup (7 Waves)

The world populates in phases to avoid overwhelming the server:

1. **Essentials** — Casino NPCs, Tutorial Bot, 10 Henesys wanderers, Henesys FM
2. **FM Buildout** — Ludi FM, merchant bots, gacha bots
3. **Henesys Population** — More wanderers, filler bots, start social simulation systems
4. **Expand FM** — Perion + Elnath FM, more merchants, Market fillers
5. **Sub-areas** — Park fillers, Potion Shop fillers, Game Zone fillers + hosts
6. **Specialty** — Blackjack tables, Drop Game, JQ bots, Pet Park social bots, scroll bot conversions
7. **Late Arrivals** — OPQ lobby bots, final merchant batch, decoration queue start
...
Full reference: `docs/Dev Commands Cheat Sheet.md`
...
src/main/java/
  soloMapling/            <- SoloMapling framework
    ArtificialPlayer/     <- Bot state machines, types, movement, trade, party, commands
      ...
      BotDecoratorSystem/ <- Procedural appearance generation (body, equips, NX)
      BotDialoguePack/    <- 14 YAML dialogue files
    Casino/               <- Casino chip system and WZ patching
    Environment/          <- World initialization, platform system
    FreeMarket/           <- FM economy simulation, shop generation
    MapVFX/               <- Reactor-based visual effects
    itemPool/             <- YAML-driven item registry and scroll system
...
- ~162 Java source files under `soloMapling/`
- 825+ movement recording files (binary + CSV across 20+ map directories)
- ~31 YAML configuration files (14 dialogue, 13 item configs, 2 version configs, 2 pool configs)
- ~12 text data files (FM shop names/descriptions)
